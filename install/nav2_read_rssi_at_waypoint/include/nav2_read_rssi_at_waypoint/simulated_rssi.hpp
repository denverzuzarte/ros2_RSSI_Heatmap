  ...
  double n = 3.0)
{
  ...
  // TODO Define parameters: Mean = 0.0, Standard Deviation = 3.0
  
  double mean = 0.0;
  double std_dev = 3.0;
  std::normal_distribution<double> noise(mean, std_dev);

  double rssi = tx_power - 10 * n * log10(dist / d0) + noise(gen);
  rssi = std::round(rssi);
